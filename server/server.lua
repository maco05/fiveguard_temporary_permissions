...
local function printServerMessage(message, color)
    ...
    return string.format("  %s%s\27[0m", color_code, message)
end

-- Table to store messages before printing them
local messageBuffer = {}
local function bufferServerMessage(message, color)
    table.insert(messageBuffer, printServerMessage(message, color))
end
...
if Config.FiveguardName == "auto" then
    ...
    if not Fiveguard then
        ...
        print("===========================================\n")
        -- Stop execution here if FiveGuard is not found via auto-detection
        return
    else
        FiveguardFound = true
    end
...
-- Only proceed with FiveGuard-related logic if Fiveguard is identified
if Fiveguard then
    if FiveguardFound then -- Only print this if it was found via auto-detection
        bufferServerMessage('Found FiveGuard with name: \27[32m' .. Fiveguard .. '\27[0m', "\27[0m")
    else -- If Config.FiveguardName was manually set and valid
        bufferServerMessage('Using FiveGuard with configured name: \27[32m' .. Fiveguard .. '\27[0m', "\27[0m")
    end
    ...
    if GetResourceState(Fiveguard) == 'started' then
        bufferServerMessage('FiveGuard linked ^2successfully^0!', "\27[32m")
    else
        StartResource(Fiveguard)
        bufferServerMessage(('Seems like you didn\'t start ^3%s^1 before this resource'):format(Fiveguard), "\27[33m")
        bufferServerMessage(('Make sure to start ^3%s^1 as first resource in your server.cfg for better compatibility with your scripts!'):format(Fiveguard), "\27[33m")
        bufferServerMessage(('Trying to start ^3%s^0 (attempt: %d)^0'):format(Fiveguard, attempts), "\27[34m")
        attempts = attempts + 1
        ...
        bufferServerMessage(('Failed to start ^3%s^1 (attempts: %d)'):format(Fiveguard, attempts), "\27[31m")
        print(table.concat(messageBuffer, "\n")) -- Print buffered messages before returning
        print("===========================================\n")
        return
    end

    -- Print all buffered messages related to FiveGuard linking at once
    print(table.concat(messageBuffer, "\n"))
    print("===========================================\n")
else
    -- If Config.FiveguardName was not "auto" and was nil or invalid, but not caught by auto-detection
    -- (e.g., manually set to a non-existent name)
    printServerMessage('FiveGuard not configured or found. Addon will not function.', "\27[31m")
    print("===========================================\n")
    return
end
...
Citizen.CreateThread(function()
    ...
    for key, cfg in pairs(Config) do
        -- Skip FiveguardName and ensure it's a table
        if key ~= "FiveguardName" and type(cfg) == "table" then
            ...
            if not hasError then
                -- Ensure Fiveguard is defined before attempting to use it
                local fiveguardName = Fiveguard
                if not fiveguardName then
                    printServerMessage(string.format("[FIVEGUARD CONFIG WARNING]: Cannot register events for section '%s' as FiveGuard was not found.", key), "\27[33m")
                else
    ...
    -- Only print preconfigured options if FiveGuard was successfully linked
    if Fiveguard then
        local presetConfigs = {
